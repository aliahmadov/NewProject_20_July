...
");
                    ...
                    if (secim == "1")
                    {
                        ...
                        Console.WriteLine("Muraciet edenlere baxmaq ucun elan ID-si daxil edin: ");
                        string ad_id = Console.ReadLine();
                        var vacancy = Helper.GetVacanciesByID(employer, Convert.ToInt32(ad_id));
                        Console.Clear();
...
");
                    ...
                    else if (secim == "2")
                    {
                        ...
                        var employer1 = Helper.GetEmployer(humans, Convert.ToInt32(id));
                        var vacancy = Helper.GetVacanciesByID(employer1, Convert.ToInt32(id));

                        Console.Clear();
                        worker.ShowCvs();
                        Console.WriteLine("Muraciet etmek istediyiniz CV-nin ID-ni daxil edin: ");
                        string cv_id = Console.ReadLine();
                        var cv = Helper.GetCVByID(worker, Convert.ToInt32(cv_id));

                        Applier applier = new Applier { Cv = cv, worker = worker };
                        vacancy.AddApplier(applier);
                        employer1.UpdateVacancy(vacancy);
                        Console.ForegroundColor = ConsoleColor.Green; Console.WriteLine($"{vacancy.Id} ID-si olan elana ugurla muraciet edildi"); Console.ResetColor();
                        Console.ReadKey();
